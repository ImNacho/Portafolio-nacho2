...
function getHelpText(lang: string): string {
  const lines = lang === 'en'
    ? [
        ...
        '  /profile               — About Hernán',
        '  /hire                  — Why hire Hernán',
...
export function processChatCommand(
  ...
  if (cmdBase === '/clear') {
    try { localStorage.removeItem('nachobot-history'); } catch { /* ignore */ }
  ...
  if (cmdBase === '/cv') {
    ...
    link.href = lang === 'en' ? '/Hoja%20de%20VIDA%20(ENG)%20ING.pdf' : '/Hoja%20de%20VIDA%20(ESP)%20ING.pdf';
    link.download = '';
