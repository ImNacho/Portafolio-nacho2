...
export default function ProjectDetail({ slug }: Props) {
  ...
  const hasProjectImages = (staticData.images?.length ?? 0) > 0;
  ...
  return (
    <div ref={ref} className="min-h-screen">
      ...
      {!hasProjectImages && (
        <div className="container mx-auto px-4 md:px-6 pt-2 md:pt-4">
          <div className="detail-image max-w-5xl mx-auto">
            <div className="rounded-[28px] border border-border/50 bg-foreground/[0.03] p-8 md:p-10 text-center shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
              <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full border border-border/50 bg-background text-xl font-black text-foreground/70">
                {project.title.charAt(0)}
              </div>
              <p className="text-xs font-mono uppercase tracking-[0.28em] text-muted-foreground">
                {lang === 'en' ? 'Project preview' : 'Vista del proyecto'}
              </p>
            </div>
          </div>
        </div>
