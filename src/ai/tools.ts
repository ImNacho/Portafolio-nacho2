...
export function getToolsDefinition(lang: string) {
  ...
  showProject: {
    description: es
      ? 'Muestra una tarjeta visual interactiva de un proyecto de Hernán. Llamar SOLO cuando el usuario pide explícitamente VER un proyecto. Para mostrar TODOS los proyectos, llamar esta función UNA VEZ POR CADA proyecto.'
      : 'Shows an interactive visual card of an Hernán project. Call ONLY when the user explicitly wants to SEE a project. To show ALL projects, call this function ONCE PER project.',
  ...
  showContact: {
    description: es
      ? 'Muestra una tarjeta interactiva con todos los medios para contactar a Hernán: LinkedIn, email, CV y portfolio. Llamar cuando alguien pregunta cómo contactar o contratar a Hernán.'
      : 'Shows an interactive card with all ways to contact Hernán: LinkedIn, email, CV and portfolio. Call when someone asks how to contact or hire Hernán.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
    execute: async () => ({
      linkedin:  'https://www.linkedin.com/in/hernan-josé-mendoza-ibáñez-229332212/',
      email:     'andromixm@gmail.com',
      cvEs:      'https://jackshaw32.dev/Hoja%20de%20VIDA%20(ESP)%20ING.pdf',
      cvEn:      'https://jackshaw32.dev/Hoja%20de%20VIDA%20(ENG)%20ING.pdf',
      portfolio: 'https://jackshaw32.dev/',
      phone:     '+57 3113845246',
      github:    'https://github.com/ImNacho',
      x:         'https://x.com/yonachox',
      instagram: 'https://www.instagram.com/soynachooo/',
    }),
  },
  ...
  showSkills: {
    description: es
      ? 'Muestra una visualización del tech stack de Hernán con badges organizados por categoría. Llamar cuando preguntan por tecnologías, habilidades o conocimientos técnicos.'
      : 'Shows a visual of Hernán\'s tech stack with badges organized by category. Call when asked about technologies, skills or technical knowledge.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
  ...
  showExperience: {
    description: es
      ? 'Muestra un timeline visual con la experiencia profesional y educación de Hernán. Llamar cuando preguntan por experiencia, trayectoria, o historial laboral.'
      : 'Shows a visual timeline of Hernán\'s professional experience and education. Call when asked about experience, career, or work history.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
  ...
  showRecommendation: {
    description: es
      ? 'Muestra una tarjeta de recomendación profesional explicando por qué contratar a Hernán, para qué tipo de proyectos es ideal, y qué lo hace destacar. Llamar cuando preguntan "por qué contratarlo", "recomiéndame", "would you recommend him", "why should I hire", "es bueno para", "is he a good fit", o evaluaciones de su perfil.'
      : 'Shows a professional recommendation card explaining why to hire Hernán, what projects he\'s ideal for, and what makes him stand out. Call when asked "why hire him", "would you recommend", "is he a good fit", or profile evaluations.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
  ...
  showArchitecture: {
    description: es
      ? 'Muestra un diagrama visual de la arquitectura técnica de los proyectos de Hernán o del portfolio en sí. Llamar cuando preguntan "arquitectura", "cómo está estructurado", "diagrama", "flujo", "architecture", "stack diagram", o cómo se conectan los componentes.'
      : 'Shows a visual diagram of the technical architecture of Hernán\'s projects or the portfolio itself. Call when asked about architecture, diagram, flow, how components connect.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
  ...
  showProfile: {
    description: es
      ? 'Muestra un resumen completo del perfil profesional de Hernán: rol, ubicación, stack principal, enfoque y disponibilidad. Llamar cuando preguntan quién es, qué hace, su perfil en general, o "contame sobre vos".'
      : 'Shows a complete summary of Hernán\'s professional profile: role, location, main stack, focus and availability. Call when asked who he is, what he does, his profile in general.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
  ...
  showAvailability: {
    description: es
      ? 'Muestra el estado actual de disponibilidad de Hernán para nuevas oportunidades laborales. Llamar cuando preguntan si está disponible, buscando trabajo, o cuándo puede empezar.'
      : 'Shows Hernán\'s current availability for new job opportunities. Call when asked if he\'s available, looking for work, or when he can start.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
  ...
  showImpact: {
    description: es
      ? 'Muestra una tarjeta con las métricas de impacto, logros y Lighthouse scores de Hernán. Llamar cuando preguntan por impacto, métricas, KPIs, logros, números o resultados profesionales.'
      : 'Shows a card with Hernán\'s impact metrics, achievements and Lighthouse scores. Call when asked about impact, metrics, KPIs, achievements, numbers or professional results.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
  ...
  sendContactForm: {
    description: es
      ? 'Envía un formulario de contacto a Hernán con nombre, email y mensaje del recruiter/cliente. Llamar SOLO cuando el usuario ya proporcionó explícitamente su nombre, email y mensaje.'
      : 'Sends a contact form to Hernán with the recruiter/client\'s name, email and message. Call ONLY when the user has explicitly provided their name, email and message.',
    inputSchema: jsonSchema<{ name: string; email: string; message: string }>({
      ...
      properties: {
        ...
        message: { type: 'string', description: es ? 'Mensaje para Hernán' : 'Message for Hernán' },
      },
  ...
  showCommentForm: {
    description: es
      ? 'Muestra un formulario para dejar un comentario o reseña sobre Hernán. Llamar cuando el usuario quiere dejar un comentario, review, testimonio o valoración.'
      : 'Shows a form to leave a comment or review about Hernán. Call when the user wants to leave a comment, review, testimonial or rating.',
    inputSchema: jsonSchema<Record<string, never>>({
      type: 'object',
      properties: {},
    }),
